...
if (!isset($_SESSION["user"]['account'])) {
    header('Location: signIn.php?error=no-account-found');
    die();
}

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if ($_POST["email"] == $_SESSION["user"]['account']['email'] && $_POST["password"] == $_SESSION["user"]['account']['password']) {
    ...
    <main>
        ...
        <form action="" method="POST" style="width: 30%;display:grid;margin:20px auto;">
            ...
            <div class="formRow" style="display: grid;">
                <button type="submit"><span style="font-size:20px;">→ </span>Connexion</button>
            </div>
        </form>
    </main>
</body>
